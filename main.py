...
def main():
    ...
    for model_name, model_fn in model_variants.items():
        for strategy in ["freeze", "finetune"]:
            ...
            if strategy == "freeze":
                for name, param in model.named_parameters():
                    if "linear1" in name or "dyt" in name or "ln" in name:
            ...
            try:
                inputs, _ = next(iter(cifar_test))
                inputs = inputs.to(config['device'])
                _ = model(inputs)
                attn = model.blocks[0][0].attn_weights
                if attn is not None:
                    plot_attention_heads(attn, os.path.join(config['save_dir'], f"{prefix}_attention.png"))
            except Exception as e:
                print(f"[WARN] Skipping attention plot: {e}")
